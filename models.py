...
class ImageDetectionResult(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # Can be null for anonymous detections
    image_filename = db.Column(db.String(255), nullable=True)
    is_ai_generated = db.Column(db.Boolean, nullable=False)
    confidence_score = db.Column(db.Integer, nullable=False)  # 0-100
    status = db.Column(db.String(50), nullable=False)  # 'AI-generated' or 'Real'
    reasons = db.Column(db.Text, nullable=True)  # Stored as JSON
    artifacts = db.Column(db.Text, nullable=True)  # Stored as JSON
